...
lacy_preheat_reset_session() {
    # Clear current shell state
    LACY_PREHEAT_SERVER_SESSION_ID=""
    LACY_PREHEAT_CLAUDE_SESSION_ID=""
    LACY_GEMINI_SESSION_ID=""
    
    # Remove current session files (scoped to this PID)
    rm -f "$LACY_PREHEAT_SERVER_SESSION_FILE" "$LACY_PREHEAT_SESSION_FILE" "$LACY_GEMINI_SESSION_ID_FILE"
}

# Resume the most recent session from any PID
lacy_preheat_resume_most_recent_session() {
    local most_recent_file=""
    
    # 1. Claude
    most_recent_file=$(ls -t "${LACY_SHELL_HOME}/.claude_session_id_"* 2>/dev/null | head -1)
    if [[ -n "$most_recent_file" && "$most_recent_file" != "$LACY_PREHEAT_SESSION_FILE" ]]; then
        LACY_PREHEAT_CLAUDE_SESSION_ID=$(cat "$most_recent_file" 2>/dev/null)
        echo "$LACY_PREHEAT_CLAUDE_SESSION_ID" > "$LACY_PREHEAT_SESSION_FILE"
    fi
    
    # 2. Gemini
    most_recent_file=$(ls -t "${LACY_SHELL_HOME}/.gemini_session_id_"* 2>/dev/null | head -1)
    if [[ -n "$most_recent_file" && "$most_recent_file" != "$LACY_GEMINI_SESSION_ID_FILE" ]]; then
        LACY_GEMINI_SESSION_ID=$(cat "$most_recent_file" 2>/dev/null)
        echo "$LACY_GEMINI_SESSION_ID" > "$LACY_GEMINI_SESSION_ID_FILE"
    fi
    
    # 3. Server (lash/opencode)
    most_recent_file=$(ls -t "${LACY_SHELL_HOME}/.server_session_id_"* 2>/dev/null | head -1)
    if [[ -n "$most_recent_file" && "$most_recent_file" != "$LACY_PREHEAT_SERVER_SESSION_FILE" ]]; then
        LACY_PREHEAT_SERVER_SESSION_ID=$(cat "$most_recent_file" 2>/dev/null)
        echo "$LACY_PREHEAT_SERVER_SESSION_ID" > "$LACY_PREHEAT_SERVER_SESSION_FILE"
    fi
}
